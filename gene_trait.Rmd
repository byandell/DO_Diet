...
```{r}
if(normscore) {
  gene_trait <- foundr::nqrank(gene_trait)
}
```
...
```{r}
fit <- lm(Wfdc1 ~ wave + Sex * Diet.name * geno, dat)
```
...
```{r}
fit <- lm(Wfdc1 ~ wave + Sex * Diet.name * (add + dom), dat)
```
